...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/wayangkulit95/REGISTER/main/IPVPS > /root/tmp
...
red "Skrip anda perlu dikemas kini terlebih dahulu !"
exit 0
elif [ "$res" = "Kebenaran diterima..." ]; then
echo -ne
else
red "Kebenaran ditolak!"
exit 0
fi
...
[[ $EUID -ne 0 ]] && echo -e "${red}Error:${plain} Skrip ini mesti dijalankan sebagai root!" && exit 1

[[ -d "/proc/vz" ]] && echo -e "${red}Error:${plain} VPS anda berdasarkan OpenVZ, yang tidak disokong." && exit 1
...
display_menu(){
    ...
    local hint
    local vname
    local prompt="yang ${soft} anda akan pilih ${default_prompt}: "
    ...
    do
        ...
        if ! is_digit "$pick"; then
            prompt="Ralat input, sila masukkan nombor"
            continue
        fi

        if [[ "$pick" -lt 1 || "$pick" -gt ${#arr[@]} ]]; then
            prompt="Ralat input, sila masukkan nombor antara 1 dan ${#arr[@]}: "
            continue
        fi
...
get_latest_version() {
    ...
    [ ${#latest_version[@]} -eq 0 ] && echo -e "${red}Ralat:${plain} Dapatkan versi kernel terkini gagal." && exit 1
    ...
    [ -z ${deb_name} ] && echo -e "${red}Error:${plain} Mendapatkan nama pakej binari kernel Linux gagal, mungkin kernel membina gagal.Sila pilih yang lain dan cuba lagi." && exit 1
}
...
install_elrepo() {
    ...
    if [ ! -f /etc/yum.repos.d/elrepo.repo ]; then
        echo -e "${red}Ralat:${plain} Pasang Elrepo gagal, sila periksa."
        exit 1
    fi
}
